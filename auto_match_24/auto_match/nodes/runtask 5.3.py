...
class ArmAction:
    ...
    def detectorD(self):
        ...
        print(cube_list1)
        if len(cube_list1) > 3:                                 ##提取第一行
            for i in range(len(cube_list1) - 1):
                if cube_list1[i][1][1] - cube_list1[i+1][1][1] < 50:
                    cube_list2.append(cube_list1[i])
                else:
                    break
        else:
            cube_list2 = cube_list1
        for i in range(len(cube_list2)):                      ##提取第一行D的个数
            if cube_list2[i][0] == object_list[2][0] and cube_list2[i][1][1] >= 230:
                a += 1
    ...
    def detectorC(self):
        ...
        for i in range(len(cube_list2)):
            if cube_list2[i][0] == object_list[1][0] and cube_list2[i][1][1] >= 230:
                a += 1
                cube_list4.append(cube_list2[i])
        # if cube_list1[0][0] == object_list[2][0] and cube_list1[0][1][1] >= 230:
        #     a += 1
    ...
    def detectorB(self):
        ...
        if len(cube_list1) > 3:
            ...
            cube_list2 = cube_list1
        for i in range(len(cube_list2)):
            if cube_list2[i][0] == object_list[0][0] and cube_list2[i][1][1] >= 230:
                a += 1
                cube_list4.append(cube_list2[i])
        if cube_list4 == []:
               cube_list4.append([5,[122,0,0]])
    ...
    def detectorend(self):
        ...
        cube_list1 = sorted(cube_list1, key=lambda y: y[1][1], reverse=True)
        print(cube_list1)
        if cube_list1[0][0] == object_list[0][0]:
            return cube_list1
        if cube_list1[0][0] == object_list[1][0]:
            return cube_list1
        if cube_list1[0][0] == object_list[2][0]:
            return cube_list1
    def grasp(self,name):
        '''
        ...
        #print(cube_list1)
        # 获取机械臂目标位置
        x = self.x_kb[0] * (name[0][1][1])+ self.x_kb[1]
...
class AutoAction:
    ...
    def run_task(self):
        ...
        # ===== 导航到分类区 =====
        if self.robot.goto_local("fenlei"):
            rospy.sleep(1)
            #pass
        else :
...
##在一点识别D
        ...
        while b > 0:
            ...
            if b != 0:
                ...
                if self.stop_flag: return   
##若没有D则顺路抓C    
        if b == 0 and c == 0:   #为达成上述循环与此if执行一个的效果，定义C标志位，来实现此效果。
...
#第二圈在yi点抓C
        ...
        while b >0:
            ...
            self.arm.arm_grasp_ready()
            print("========前往放置区===== ")
            ret = self.robot.goto_local("er") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
            if self.stop_flag: return 
            ret = self.robot.goto_local("Collection_C") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
...
#第二圈在san点抓B
        ...
        while b >0:
            ...
            self.arm.arm_grasp_ready()
            print("========前往放置区===== ")
            ret = self.robot.goto_local("si") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
            if self.stop_flag: return 
            ret = self.robot.goto_local("Collection_B") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
...
#第二圈在qi点抓C
        ...
        while b >0:
            ...
            self.arm.arm_grasp_ready()
            print("========前往放置区===== ")
            ret = self.robot.goto_local("liu") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
            if self.stop_flag: return 
            ret = self.robot.goto_local("Collection_C") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
            ...
            b = b - 1
            if b != 0:
                print("返回zhongjiandian点")
                ret = self.robot.goto_local('zhongjiandian')
                if self.stop_flag: return 
#第二圈在zhongjiandian点抓中心的物体
        item_type = self.arm.detectorend()  
        print("第二圈在zhongjiandian点抓中心的物体c")
        print("========扫描中，准备抓取===== ")
        self.arm.grasp(item_type)  # 抓取物品并返回抓取物品的类型   
        if item_type[0][0] == object_list[0][0]:
            ret = self.robot.goto_local("Collection_B") # 根据抓到的物品类型，导航到对应的放置区
