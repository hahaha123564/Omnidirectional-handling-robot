...
class ArmAction:
    ...
    def detectorD(self):
        ...
        for i in range(len(cube_list1)):
            if cube_list1[i][1][1] - cube_list1[i+1][1][1] < 60:
    ...
    def drop2(self, check=False):
        ...
        if cube_list0 != []:
            ...
            if len(cube_list3) > 0:
                x = self.x_kb[0] * (cube_list3[0][1][1] - 20) + self.x_kb[1] 
                y = self.y_kb[0] * cube_list3[0][1][0] + self.y_kb[1]
                z = 100
        self.interface.set_pose(70, 0, 60)
        # 默认放置位置
...
#第二圈在yi点抓C
        ...
        while b > 0 and yi != 1:
            ...
            self.arm.arm_grasp_ready()
            print("========前往放置区===== ")
            ret = self.robot.goto_local("ery") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
            if self.stop_flag: return 
            ret = self.robot.goto_local("Collection_C") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
            ...
            if b != 0:
                ...
                ret = self.robot.goto_local('yi')
                if self.stop_flag: return 
        if yi == 1 and san == 1 and wu == 1 and qi == 1:
            print("返回zhongjiandian点")
            ret = self.robot.goto_local('zhongjiandian2')
            if self.stop_flag: return
...
#第二圈在san点抓B
        ...
        while b > 0 and san != 1:
            ...
            self.arm.arm_grasp_ready()
            print("========前往放置区===== ")
            ret = self.robot.goto_local("six") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
            if self.stop_flag: return 
            ret = self.robot.goto_local("Collection_B") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
...
#第二圈在wu点抓D
        ...
        while b > 0 and wu != 1:
            ...
            self.arm.arm_grasp_ready()
            print("========前往放置区===== ")
            ret = self.robot.goto_local("siy") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
            if self.stop_flag: return 
            ret = self.robot.goto_local("Collection_D") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
            ...
            if b != 0:
                ...
                if self.stop_flag: return 
        if yi == 1 and san == 1 and wu == 1 and qi == 1:
            print("返回zhongjiandian1点")
            ret = self.robot.goto_local('zhongjiandian1')
            if self.stop_flag: return
            item_type = self.arm.detectorend()  
            print("第二圈在zhongjiandian点抓中心的物体c")
            print("========扫描中，准备抓取===== ")
...
#第二圈在qi点抓C
        ...
        if ret: 
            ...
            b = a
        if qi == 0:                                  
            ret = self.robot.goto_local('zhongxindian1')
            if self.stop_flag: return
        while b > 0 and qi != 1:
            if b == 3:
                print("3个C")
            ...
            self.arm.arm_grasp_ready()
            print("========前往放置区===== ")
            ret = self.robot.goto_local("liux") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
            if self.stop_flag: return 
            ret = self.robot.goto_local("Collection_C") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
