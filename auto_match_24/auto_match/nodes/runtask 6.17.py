...
class ArmAction:
    ...
    def detectorD(self):
        ...
        while True:
            ...
            for i in range(len(cube_list1)):
                if cube_list1[i][1][1] - cube_list1[i+1][1][1] < 55:
            ...
            if cube_list4 == []:
                break   
            elif cube_list4[0][1][0] < 128 or cube_list4[0][1][0] > 508:  
                if cube_list4[0][1][0] < 128:
                    print("前进")
                    self.robot.rotate_right(10)
                    rospy.sleep(0.5)
                    cube_list2 = []
                    cube_list4 = []
                    if self.stop_flag: return
                elif cube_list4[0][1][0] > 508:
                    print("后退")
                    self.robot.rotate_left(10)
                    rospy.sleep(0.5)
                    cube_list2 = []
                    cube_list4 = []
                    if self.stop_flag: return               
            elif cube_list4[0][1][1] < 240 or cube_list4[0][1][1] > 280:
                if cube_list4[0][1][1] < 240:
                    print("前进")
                    self.robot.step_go1()
                    rospy.sleep(0.5)
                    cube_list2 = []
    ...
    def drop2(self, check=False):
        ...
        if cube_list0 != []:
            ...
            while True:
                ...
                if cube_list3[0][1][1] < 240 or cube_list3[0][1][1] > 310:
                    if cube_list3[0][1][1] < 240:
                        print("前进")
                        self.robot.step_go1()
                        rospy.sleep(0.5)
                        if self.stop_flag: return
                    elif cube_list3[0][1][1] > 310:
                        print("后退")
                        self.robot.step_back()
                        rospy.sleep(0.5)
                        if self.stop_flag: return
    ...
    def drop3(self, check=False):
        ...
        if cube_list0 != []:
            ...
            while True:
                cube_list3 = cube_list0
                cube_list3 = sorted(cube_list3, key=lambda x: x[1][0], reverse=True)
                if cube_list3[0][1][1] < 240 or cube_list3[0][1][1] > 290:
                    if cube_list3[0][1][1] < 240:
                        self.robot.step_go1()
                        rospy.sleep(0.5)
                        if self.stop_flag: return
                    elif cube_list3[0][1][1] > 290:
                        self.robot.step_back()
                        rospy.sleep(0.5)
                        if self.stop_flag: return
                else:
        ...
        self.interface.set_pose(220, 0, 190) 
        self.interface.set_pose(x, y, z)
        rospy.sleep(1)
        ...
        cube_list3 = sorted(cube_list3, key=lambda x: x[1][0], reverse=True)
        if len(cube_list3) == 3:
            x = self.x_kb[0] * (cube_list3[2][1][1])+ self.x_kb[1]
            y = self.y_kb[0] * (cube_list3[2][1][0]) + self.y_kb[1]
            z = -55           
            self.interface.set_pose(150, 0, 10)
            print(f"找到物品了！它在: {x}, {y}, {z}")
            rospy.sleep(0.5)
            ...
            cube_list3 = cube_list0
            cube_list3 = sorted(cube_list3, key=lambda x: x[1][0], reverse=True)
            x = self.x_kb[0] * (cube_list3[1][1][1])+ self.x_kb[1]
            y = self.y_kb[0] * (cube_list3[1][1][0]) + self.y_kb[1]
            z = -55           
            self.interface.set_pose(150, 0, 10)
            print(f"找到物品了！它在: {x}, {y}, {z}")
            rospy.sleep(0.5)
    ...
    def arm_grasp_ready(self, block=False):
        ...
        '''
        self.interface.set_pose(30, 180, 160)  
        if block:
            rospy.sleep(0.5)
...
class RobotMoveAction:
    ...
    def rotate_left(self, angle_degrees):
        '''
        左旋指定角度
        @param angle_degrees: 旋转的角度（度）
        @return: True 为旋转成功, False 为旋转失败
        '''
        self.turn_action_cli.send_goal_and_wait(
            common.msg.TurnBodyDegreeActionGoal(
                target_degree=angle_degrees,
                clockwise=False,  # 顺时针为True，逆时针为False，这里表示左旋即逆时针
            ),
            rospy.Duration.from_sec(5)  # 超过5s为超时
        )
        return True
    ...
    def rotate_right(self, angle_degrees):
        ...
        self.turn_action_cli.send_goal_and_wait(
            common.msg.TurnBodyDegreeActionGoal(
                target_degree=angle_degrees,
                clockwise=True,  # 顺时针为True，逆时针为False，这里表示右旋即顺时针
            ),
            rospy.Duration.from_sec(5)  # 超过5s为超时
        )
        return True
...
##在一点识别B
        ...
        while b > 0 :
            ...
            print("========前往放置区===== ")
            ret = self.robot.goto_local("bay") # 根据抓到的物品类型，导航到对应的放置区
            #rospy.sleep(2) # 停稳
            if self.stop_flag: return  
